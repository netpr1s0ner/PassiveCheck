...
func main() {
	...
	// 加载指纹信息
	runner.FingersFile = options.FingerprintDict
	runner.Run()

	// 输出指纹信息
	if options.ShowFingerprint > 0 {
		runner.ShowFingerprintHub(options.ShowFingerprint)
		return
	} else if options.UpdateFingerprint {
		// 指纹更新
		err := runner.UpdateFingerprintHub(runner.FingersFile)
		if err != nil {
			fmt.Printf("[-] 更新指纹库失败: %s\n", err)
		} else {
			fmt.Printf("[+] 更新指纹库成功: file save success in %s\n", runner.FingersFile)
		}
		return
	}
