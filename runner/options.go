...
type Options struct {
	...
	DirDict                   string
	DelayTimme                int
	FilteredInterferenceData  bool
	UpdateFingerprint         bool
	ShowFingerprint           int
	FingerprintDict           string
	InputFile                 string
...
func ParseOptions() *Options {
	...
	flagSet.SetDescription(`PassiveCheck is a fast and multi-purpose HTTP toolkit that allows running multiple probes using the retryablehttp library.`)
	flagSet.CreateGroup("Target", "Input Target",
		flagSet.StringSliceVarP(&options.InputTargetHost, "target", "t", nil, "input single target", goflags.CommaSeparatedStringSliceOptions),
		flagSet.BoolVarP(&options.Paste, "paste", "p", false, "Input targets from clipboard"),
	...
	flagSet.CreateGroup("Sub Dir Scan", "Low thread to scan website sub path",
		flagSet.BoolVarP(&options.DirScan, "dirScan", "ds", false, "Second Directory Scanning"),
		flagSet.BoolVarP(&options.DirScanDeep, "deepDirScan", "dds", false, "Deep Second Directory Scanning"),
		flagSet.StringVarP(&options.DirDict, "dirDict", "dc", "", "Directory scan dictionary"),
		flagSet.IntVarP(&options.DelayTimme, "delayTime", "dl", 0, "delay time to scan second directory"),
		flagSet.BoolVarP(&options.FilteredInterferenceData, "Filtered interference data", "fd", true, "Filtered interference data"),
	)

	flagSet.CreateGroup("fingerprint identification", "website fingerprint identification",
		flagSet.IntVarP(&options.ShowFingerprint, "ShowFingerprint", "sf", -1, "Print detailed fingerprint information"),
		flagSet.BoolVarP(&options.UpdateFingerprint, "UpdateFingerprint", "uf", false, "Update fingerprint library"),
		flagSet.StringVarP(&options.FingerprintDict, "FingerprintDict", "ff", "FingerprintHub.txt", "Specify the fingerprint library file"),
	)

	flagSet.CreateGroup("Probes", "Probes",
		flagSet.BoolVarP(&options.StatusCode, "status-code", "sc", true, "display response status-code"),
		flagSet.BoolVarP(&options.ContentLength, "content-length", "cl", true, "display response content-length"),
		...
		flagSet.StringVar(&options.Hashes, "hash", "", "display response bodyType hash (supported: md5,mmh3,simhash,sha1,sha256,sha512)"),
		...
		flagSet.BoolVarP(&options.OutputLinesCount, "line-count", "lc", false, "display response bodyType line count"),
		flagSet.BoolVarP(&options.OutputWordsCount, "word-count", "wc", false, "display response bodyType word count"),
		flagSet.BoolVar(&options.ExtractTitle, "title", true, "display page title"),
		flagSet.DynamicVarP(&options.ResponseBodyPreviewSize, "bodyType-preview", "bp", 100, "display first N characters of response bodyType"),
		flagSet.BoolVarP(&options.OutputServerHeader, "web-server", "server", true, "display server name"),
		flagSet.BoolVarP(&options.TechDetect, "tech-detect", "td", true, "display technology in use based on wappalyzer dataset"),
		flagSet.BoolVar(&options.OutputMethod, "method", true, "display http request method"),
		flagSet.BoolVar(&options.OutputWebSocket, "websocket", false, "display server using websocket"),
		flagSet.BoolVar(&options.OutputIP, "ip", true, "display host ip"),
		flagSet.BoolVar(&options.OutputCName, "cname", true, "display host cname"),
		flagSet.BoolVarP(&options.ExtractFqdn, "efqdn", "extract-fqdn", false, "get domain and subdomains from response bodyType and header in jsonl/csv output"),
	...
	flagSet.CreateGroup("headless", "Headless",
		...
		flagSet.BoolVarP(&options.NoHeadlessBody, "exclude-headless-bodyType", "ehb", false, "enable excluding headless header from json output"),
		flagSet.IntVarP(&options.ScreenshotTimeout, "screenshot-timeout", "st", 15, "set timeout for screenshot in seconds"),
	)
	...
	flagSet.CreateGroup("matchers", "Matchers",
		...
		flagSet.StringVarP(&options.OutputMatchLinesCount, "match-line-count", "mlc", "", "match response bodyType with specified line count (-mlc 423,532)"),
		flagSet.StringVarP(&options.OutputMatchWordsCount, "match-word-count", "mwc", "", "match response bodyType with specified word count (-mwc 43,55)"),
	...
	flagSet.CreateGroup("filters", "Filters ",
		flagSet.StringVarP(&options.OutputFilterStatusCode, "filter-code", "fc", "404", "filter response with specified status code (-fc 403,401)"),
		...
		flagSet.StringVarP(&options.OutputFilterLinesCount, "filter-line-count", "flc", "", "filter response bodyType with specified line count (-flc 423,532)"),
		flagSet.StringVarP(&options.OutputFilterWordsCount, "filter-word-count", "fwc", "", "filter response bodyType with specified word count (-fwc 423,532)"),
	...
	flagSet.CreateGroup("rate-limit", "Rate-Limit",
		flagSet.IntVarP(&options.Threads, "threads", "th", defaultThreads, "number of threads to use"),
	...
	flagSet.CreateGroup("update", "Update",
		flagSet.BoolVarP(&options.DisableUpdateCheck, "disable-update-check", "duc", true, "disable automatic httpx update check"),
	)

	flagSet.CreateGroup("output", "Output",
		flagSet.StringVarP(&options.Output, "output", "o", "ResponseResult.txt", "file to write output results"),
		...
		flagSet.BoolVarP(&options.OmitBody, "omit-bodyType", "ob", false, "omit response bodyType in output"),
		...
		flagSet.BoolVarP(&options.ResponseInStdout, "include-response", "irr", false, "include http request/response (headers + bodyType) in JSON output (-json only)"),
	...
	flagSet.CreateGroup("configs", "Configurations",
		...
		flagSet.BoolVarP(&options.FollowRedirects, "follow-redirects", "fr", true, "follow http redirects"),
		...
		flagSet.StringVar(&options.RequestBody, "bodyType", "", "post bodyType to include in http request"),
		...
		flagSet.BoolVar(&options.NoDecode, "no-decode", false, "avoid decoding bodyType"),
	...
	flagSet.CreateGroup("Optimizations", "Optimizations",
		...
		flagSet.IntVar(&options.Timeout, "timeout", 20, "timeout in seconds"),
...
func (options *Options) ValidateOptions() error {
	if options.Paste {
		...
		if err != nil || len(content) == 0 {
			return fmt.Errorf("clipboard read failed or target is null")
		}
	}
