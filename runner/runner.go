...
import (
	"PassiveCheck/common/customextract"
	...
	"PassiveCheck/common/hashes/jarm"
...
func cleanURL(inputUrl string) string {
	...
	if err != nil {
		...
		return parsedURL.Scheme + "://" + parsedURL.Host + path.Clean(parsedURL.Path)
	}
}
...
func (r *Runner) prepareInput() {
	var numHosts int
	// from paste get target
	if r.options.Paste {
		content, err := clipboard.ReadAll()
		if err == nil && len(content) > 0 {
			for _, target := range strings.Split(strings.ReplaceAll(strings.ReplaceAll(strings.ReplaceAll(content, "\r\n", "\n"), " ", ""), "\t", ""), "\n") {
				expandedTarget := r.countTargetFromRawTarget(target)
				if expandedTarget > 0 {
					numHosts += expandedTarget
					_ = r.hm.Set(target, nil) //nolint
				}
			}
		}
	}
...
func (r *Runner) RunEnumeration() {
	...
	go func(output chan Result, nextSteps ...chan Result) {
		...
		if r.options.Output != "" && plainFile == nil && !jsonOrCsv {
			plainFile = openOrCreateFile(r.options.Resume, r.options.Output)
			_, err := plainFile.Write([]byte(fmt.Sprintf("\n\n================================================================================  %s  ================================================================================\n", time.Now().Format("2006-01-02 15:04:05"))))
			if err != nil {
				return
			}
			defer plainFile.Close()
		...
		for resp := range output {
			...
			// 只有处于目录扫描模式下 才过滤状态码
			if (r.options.DirScan || r.options.DirScanDeep) && len(r.options.filterStatusCode) > 0 && sliceutil.Contains(r.options.filterStatusCode, resp.StatusCode) {
				continue
			}

			// 只有处于目录扫描模式下 才过滤响应包
			if (r.options.DirScan || r.options.DirScanDeep) && len(r.options.filterContentLength) > 0 && sliceutil.Contains(r.options.filterContentLength, resp.ContentLength) {
				continue
			}
				...
				for _, prot := range protocols {
					...
					go func(target httpx.Target, method, protocol string) {
						...
						scanPaths := []string{"",
							...
							"/adminer.php",
							"/nccloud/resources/uap/rbac/login/main/index.html",
							"/solr/",
							"/jmreport/",
							...
							"/cwbase/weblogin/index.aspx",
							"/K3Cloud/",
							"/K3Cloud/HTML5/script/kdUtils.min.js",
						...
						// 默认不扫描路由
						if !r.options.DirScan && !r.options.DirScanDeep {
							scanPaths = []string{""}
						} else {
							// 检查是否使用了自定义字典
							fileByte, err := os.ReadFile(r.options.DirDict)
							if err != nil {
								// 检查目录扫描模式
								if r.options.DirScanDeep {
									deepPath := []string{"/admin", "/login", "/manage", "/administrator", "/admincp", "/adminpanel", "/admin-login", "/admin_dashboard", "/admin_area", "/admin-console", "/admin_home", "/admincontrol", "/adminportal", "/cmsadmin", "/backend", "/sysadmin", "/systemadmin", "/rootadmin", "/dashboard", "/panel", "/wp-admin", "/wp-login.php", "/user/login", "/drupal/admin", "/joomla/administrator", "/typo3/backend", "/phpmyadmin", "/shop-admin", "/magento/admin", "/opencart/admin", "/prestashop/admin", "/whmcs/admin", "/oscommerce/admin", "/ecshop/admin", "/woocommerce/admin", "/vtigercrm/index.php", "/suitecrm/admin", "/oa", "/oa/admin", "/hr/admin", "/crm/admin", "/erp/admin", "/workflow/admin", "/bbsadmin", "/discuz/admin.php", "/phpbb/admin", "/vbulletin/admincp", "/api/admin", "/api/manage", "/api/dashboard", "/api/v1/admin", "/api/v2/admin", "/rest/admin", "/graphql/admin", "/server_admin", "/server_manager", "/server_control", "/phpmyadmin", "/myadmin", "/webadmin", "/cloud/admin", "/aws/admin", "/vmware/admin", "/openstack/admin", "/remote_admin", "/remotemanage", "/remote_dashboard", "/cpanel", "/plesk", "/directadmin", "/pma", "/monitor", "/reports", "/logs", "/stats", "/secure", "/config", "/webconfig", "/console", "/setup", "/install", "/update", "/maintenance", "/debug", "/dev", "/test", "/staging", "/backup", "/system"}
									scanPaths = append(scanPaths, deepPath...)
								}
							} else {
								scanPaths = []string{""}
								scanPaths = append(scanPaths, strings.Split(strings.Trim(strings.Trim(strings.Trim(string(fileByte), ""), "\t"), "\n"), "\n")...)
							}
						}
						...
						// 二级目录扫描
						for _, currentPath := range scanPaths {
							target.Host = strings.TrimRight(basicHost, "/") + currentPath
							result := r.analyze(hp, protocol, target, method, t, scanopts)
							if strings.EqualFold(currentPath, "") {
								if result.Response != nil && len(result.Response.RawHeaders) > 0 && result.Response.Data != nil {
									headerHash = hashes.Md5([]byte(result.Response.RawHeaders))
									bodyHash = hashes.Md5(result.Response.Data)
								}

							} else {
								if result.Response != nil && len(result.Response.RawHeaders) > 0 && result.Response.Data != nil {
									// 判断hash是否相同
									if strings.EqualFold(headerHash, hashes.Md5([]byte(result.Response.RawHeaders))) && strings.EqualFold(bodyHash, hashes.Md5(result.Response.Data)) {
										continue
									}
								}
							}
							...
							// 分析 favicon
							if result.Response != nil && len(result.Response.RawData) > 0 {
								// 匹配 <link> 标签，寻找 rel="icon" 或 rel="shortcut icon"
								re := regexp.MustCompile(`<link\s+[^>]*rel\s*=\s*['"][shorcut ]*icon\b[^'"]*['"][^>]*>`)

								// 查找匹配项
								matches := re.FindAllString(string(result.Response.RawData), -1)
								...
								for _, match := range matches {
									...
									hrefMatches := hrefRe.FindStringSubmatch(match)
									if len(hrefMatches) > 1 {
										urls = append(urls, hrefMatches[1])
									}
								}
								...
								for _, cUrl := range urls {
									if !strings.HasPrefix(cUrl, "http") {
										target.Host = cleanURL(target.Host + "/" + cUrl)
									} else {
										target.Host = cUrl
									}
									time.Sleep(time.Second * time.Duration(r.options.DelayTimme))
									output <- r.analyze(hp, protocol, target, method, t, scanopts)
									continueStatus = true
									break
								}
								...
								// 检查是否basic path
								pathOriginal, err := getPathFromURL(target.Host)
								if err != nil || pathOriginal != "/" {
									continue
								} else {
									time.Sleep(time.Second * time.Duration(r.options.DelayTimme))
									target.Host = cleanURL(target.Host + "/favicon.ico")
									tmpResult := r.analyze(hp, protocol, target, method, t, scanopts)
									// 判断hash是否相同
									if tmpResult.Response != nil && len(tmpResult.Response.RawHeaders) > 0 && tmpResult.Response.Data != nil && strings.EqualFold(headerHash, hashes.Md5([]byte(tmpResult.Response.RawHeaders))) && strings.EqualFold(bodyHash, hashes.Md5(tmpResult.Response.Data)) {
										continue
									}
									output <- tmpResult
								}
							...
							if result.Response != nil && len(result.Response.RawData) > 0 {
								// 匹配 <meta 标签
								re := regexp.MustCompile(`<meta\s+http-equiv=["']refresh["']\s+content=["'][^"']*url=([^\s"']+)["']`)
								matches := re.FindStringSubmatch(string(result.Response.RawData))
								if len(matches) == 2 {
									...
									if err != nil {
										...
										if !strings.HasPrefix(redirectPath, "http") {
											redirectURL = parsedURL.Scheme + "://" + parsedURL.Host + parsedURL.Path + "/" + redirectPath
										}
										target.Host = cleanURL(redirectURL)
										time.Sleep(time.Second * time.Duration(r.options.DelayTimme))
										result = r.analyze(hp, protocol, target, method, t, scanopts)
										output <- result
									}
							...
							if result.Response != nil && len(result.Response.RawData) > 0 {
								// 匹配 window onload 事件
								re := regexp.MustCompile(`window\.onload\s*=\s*function\s*\(\)\s*{([\s\S]*?)}`)
								matches := re.FindStringSubmatch(string(result.Response.RawData))
								if len(matches) == 2 {
									extractedJS := strings.TrimRight(strings.TrimRight(matches[1], "\n"), " ") // 提取的 JS 代码
									if !strings.HasSuffix(extractedJS, ";") {
										extractedJS = extractedJS + ";"
									}

									// 运行提取的 JS 代码
									vm := otto.New()
									extractedJS = strings.ReplaceAll(strings.ReplaceAll(strings.ReplaceAll(extractedJS, "window.location.href", "tempAAA"), "window.location", "tempAAA"), "location.href", "tempAAA")
									script := extractedJS + "\ntempAAA;" // 让 otto 返回 redirectUrl
									value, err := vm.Run(script)
									if err != nil {
									} else {
										redirectPath := value.String()
										// 处理相对 URL
										parsedURL, err := url.Parse(result.URL)
										if err != nil {
										} else {
							...
							if result.Response != nil && len(result.Response.RawData) > 0 {
								// 匹配 onload - 1
								re := regexp.MustCompile(`onload\s*=\s*"\s*javascript:([\s\S]*?)"`)
								matches := re.FindStringSubmatch(string(result.Response.RawData))
								if len(matches) == 2 {
									extractedJS := strings.TrimRight(strings.TrimRight(matches[1], "\n"), " ") // 提取的 JS 代码
									if !strings.HasSuffix(extractedJS, ";") {
							...
							if result.Response != nil && len(result.Response.RawData) > 0 {
								// 匹配 <script> - 1
								re := regexp.MustCompile(`<script[^>]*>([\s\S]+?location.href\s*=[\s\S]+?)</script>`)
								matches := re.FindStringSubmatch(string(result.Response.RawData))
								if len(matches) == 2 {
									extractedJS := strings.TrimRight(strings.TrimRight(matches[1], "\n"), " ") // 提取的 JS 代码
									if !strings.HasSuffix(extractedJS, ";") {
							...
							if result.Response != nil && len(result.Response.RawData) > 0 {
								// 匹配 <script> -
								re := regexp.MustCompile(`<script[^>]*>([\s\S]+?location\s*=[\s\S]+?)</script>`)
								matches := re.FindStringSubmatch(string(result.Response.RawData))
								if len(matches) == 2 {
									extractedJS := strings.TrimRight(strings.TrimRight(matches[1], "\n"), " ") // 提取的 JS 代码
									if !strings.HasSuffix(extractedJS, ";") {
									...
									// 运行提取的 JS 代码
									vm := otto.New()
									extractedJS = strings.ReplaceAll(strings.ReplaceAll(extractedJS, "window.location", "tempAAA"), "location", "tempAAA")
									script := extractedJS + "\ntempAAA;" // 让 otto 返回 redirectUrl
									value, err := vm.Run(script)
									if err != nil {
									} else {
							...
							if result.Response != nil && len(result.Response.RawData) > 0 {
								// 匹配 <script> - 2
								re := regexp.MustCompile(`<script[^>]*>([\s\S]+?location.replace\([\s\S]+?)</script>`)
								matches := re.FindStringSubmatch(string(result.Response.RawData))
								if len(matches) == 2 {
									extractedJS := strings.ReplaceAll(strings.TrimRight(strings.TrimRight(matches[1], "\n"), " "), "location.replace", "location=") // 提取的 JS 代码
									if !strings.HasSuffix(extractedJS, ";") {
										extractedJS = extractedJS + ";"
									}
									...
									// 运行提取的 JS 代码
									vm := otto.New()
									extractedJS = strings.ReplaceAll(strings.ReplaceAll(strings.ReplaceAll(extractedJS, "window.location.href", "tempAAA"), "window.location", "tempAAA"), "location", "tempAAA")
									script := extractedJS + "\ntempAAA;" // 让 otto 返回 redirectUrl
									value, err := vm.Run(script)
									if err != nil {
									} else {
							...
							if result.Response != nil && len(result.Response.RawData) > 0 {
								// 匹配 window.location.href location.href
...
func (r *Runner) analyze(hp *httpx.HTTPX, protocol string, target httpx.Target, method, origInput string, scanopts *ScanOptions) Result {
	returnStatus := false
	viewedUrlLock.Lock()
	if contains(viewedUrl, target.Host) {
		returnStatus = true
	} else {
		viewedUrl = append(viewedUrl, target.Host)
	}
	viewedUrlLock.Unlock()
	if returnStatus {
		return Result{}
	}
...
retry:
	...
	if scanopts.Unsafe {
		...
		// Create a copy on the fly of the request bodyType
		if scanopts.RequestBody != "" {
			req.ContentLength = int64(len(scanopts.RequestBody))
			req.Body = io.NopCloser(strings.NewReader(scanopts.RequestBody))
		}
		...
		// Otherwise redirects like 307/308 would fail (as they require the bodyType to be sent along)
	...
	if scanopts.OutputIP || scanopts.ProbeAllIPS {
		addr, err_ := GetIPLocation(ip)
		if err_ != nil || len(addr) == 0 {
		} else {
			ip = ip + "|" + addr
		}
	...
	if scanopts.OutputContentLength {
		builder.WriteString(" [")
		if !scanopts.OutputWithNoColor {
			builder.WriteString(aurora.White("bodyLen:" + strconv.Itoa(resp.ContentLength)).String())
		} else {
	...
	// 输出servername
	serverHeader := resp.GetHeader("Server") + resp.GetHeader("server")
	if scanopts.OutputServerHeader && len(serverHeader) > 0 {
		builder.WriteString(" [")
		if !scanopts.OutputWithNoColor {
			builder.WriteString(aurora.Green("server:" + serverHeader).String())
		} else {
			builder.WriteString(serverHeader)
		}
		builder.WriteRune(']')
	}
	...
	// 被动指纹识别 自定义
	technologies2 := fingerprintIdentification(resp.Headers, []byte(resp.Raw), req.URL.Path, resp.GetChainLastURL())

	// 被动指纹识别  指纹 https://github.com/0x727/FingerprintHub
	technologies3 := fingerprintIdentificationFingerprintHub(r, []byte(resp.Raw), resp.RawData, req.URL.Path, resp.GetChainLastURL())
	...
	if scanopts.extractRegexps != nil {
		for regex, compiledRegex := range scanopts.extractRegexps {
			matches := compiledRegex.FindAllString(resp.Raw, -1)
			if len(matches) > 0 {
	...
	if scanopts.Screenshot {
		...
		if err != nil {
			...
			// As we now have headless bodyType, we can also use it for detecting
	...
	if scanopts.TechDetect {
		if len(technologies) > 0 {
			...
			if r.options.TechDetect {
				builder.WriteString(" [")
				if !scanopts.OutputWithNoColor {
					builder.WriteString(aurora.Green(technologies).String())
				} else {
		...
		if len(technologies2) > 0 {
			sort.Strings(technologies2)
			technologies := strings.Join(technologies2, ",")
			// only print to console if tech-detect flag is enabled
			// scanopts.TechDetect implicitly enabled for json , csv and asset-upload
			if r.options.TechDetect {
				builder.WriteString(" [")
				if !scanopts.OutputWithNoColor {
					builder.WriteString(aurora.BrightRed(technologies).String())
				} else {
					builder.WriteString(technologies)
				}
				builder.WriteRune(']')
		...
		if len(technologies3) > 0 {
			sort.Strings(technologies3)
			technologies := strings.Join(technologies3, ",")
			// only print to console if tech-detect flag is enabled
			// scanopts.TechDetect implicitly enabled for json , csv and asset-upload
			if r.options.TechDetect {
				builder.WriteString(" [")
...
func (r *Runner) HandleFaviconHash(hp *httpx.HTTPX, req *retryablehttp.Request, currentResp []byte, defaultProbe bool) (string, string, string, []byte, string, error) {
	// Check if current URI is ending with .ico => use current bodyType without additional requests
